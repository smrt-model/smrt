...
def run(sensor, snowpack, dmrt_qms_path=None, snowpack_dimension=None, full_output=False):
    ...
    TbV, TbH, deg0, ot, albedo, epsr_snow = octave.DMRT_QMS_passive(
        sensor.frequency / GHz, diameter, density, stickiness, thickness, temperature, Tg, epsr_ground, rough, nout=6
    )
...
def dmrt_qms_active(sensor, snowpack):
    ...
    for deg0inc in np.degrees(sensor.theta_inc):
        res = octave.DMRT_QMS_active(
            sensor.frequency / GHz,
            float(deg0inc),
            thickness,
            density,
            diameter,
...
def dmrt_qms_emmodel(sensor, layer, dmrt_qms_path=None):
    ...
    ot, albedo, epsr_snow = octave.DMRT_QMS_coefs(
        sensor.frequency / GHz, diameter, density, stickiness, thickness, temperature, nout=3
    )
