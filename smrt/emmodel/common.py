...
def phase_matrix_from_scattering_amplitude(fvv, fvh, fhv, fhh, npol=2):
    ...
    if npol == 2:
        p = [[abs2(fvv), abs2(fvh)], [abs2(fhv), abs2(fhh)]]
    elif npol == 3:
        cfhh = np.conj(fhh)
        cfhv = np.conj(fhv)

        p = [
            [abs2(fvv), abs2(fvh), (np.conj(fvh) * fvv).real],
            [abs2(fhv), abs2(fhh), (cfhh * fhv).real],
            [
                2 * (fvv * cfhv).real,
                2 * (fvh * cfhh).real,
                (fvv * cfhh + fvh * cfhv).real,
            ],
        ]
    else:
...
def rayleigh_scattering_matrix_and_angle_maetzler06(mu_s, mu_i, dphi, npol=2):
    ...
    if npol == 2:
        ...
        RLi = np.array(
            [
                [cosT2 * Li[0][0], cosT2 * Li[0][1], cosT2 * Li[0][2]],
                Li[1],
...
def Lmatrix(cos_phi, sin_phi_sign, npol):
    ...
    if npol == (2, 3):
        ...
        L = [[cos2_phi, sin2_phi, s05],
             [sin2_phi, cos2_phi, -s05]]  # fmt: skip
    elif npol == (3, 2):
        L = [[cos2_phi, sin2_phi],
             [sin2_phi, cos2_phi],
             [-sin_2phi, sin_2phi]]  # fmt: skip
    else:  # 3 pol
        s05 = 0.5 * sin_2phi
        cos_2phi = 2 * cos2_phi - 1  # cos(2 alpha)
        L = [[cos2_phi, sin2_phi, s05],
             [sin2_phi, cos2_phi, -s05],
             [-sin_2phi, sin_2phi, cos_2phi]]  # fmt: skip
    return L
...
def derived_EMModel(base_class, effective_permittivity_model):
    ...
    return type(
        new_class_name,
        (base_class,),
        dict(effective_permittivity_model=staticmethod(effective_permittivity_model)),
    )
...
class GenericFTPhaseMixin(object):
    def ft_even_phase(self, mu_s, mu_i, m_max, npol=None, nsamples=None):
        ...
        # order is pola_s, pola_i, m, mu_s, mu_i
