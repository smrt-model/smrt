...
from smrt.inputs.make_bedrock import _BEDROCK_COMPLEX_PERMITTIVITY_DATA_TABLE, make_bedrock
...
def test_make_bedrock_named_model():
    bedrock = make_bedrock(substrate_model=Flat, bedrock_permittivity_model="granite_hartlieb16", temperature=273.15)
    assert isinstance(bedrock, Flat)
...
def test_make_bedrock_named_model_with_conductivity():
    ...
    bedrock = make_bedrock(
        substrate_model=Flat, bedrock_permittivity_model="frozen_bedrock_tulaczyk20", temperature=273.15
    )
    assert isinstance(bedrock, Flat)
...
def test_bedrock_data_consistency():
    for name, (complex_permittivity, conductivity) in _BEDROCK_COMPLEX_PERMITTIVITY_DATA_TABLE.items():
        ...
        if conductivity != 0:
            assert complex_permittivity.imag == 0, (
                f"Model '{name}': Conductivity is non-zero, but imaginary permittivity is also non-zero."
            )
