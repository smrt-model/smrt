...
.. list-table:: SMRT Solvers Overview
   ...
   * - Solver Name
     ...
     - Performance
   * - ``dort``
     - The default original solver in SMRT for passive microwave (brightness temperature) and radar (sigma0). General
       and robust.
     - General applications for both passive microwave and radar.
     - Robust but slow
   * - ``iterative_first_order``
     - Radar-only solver using an iterative method up to first order. Fast and provides contributions of interaction
       mechanisms.
...
.. admonition:: **For Developers**
    ...
    available through :py:mod:`smrt.core.model.make_model`.
    ...
    to be implemented. It must return a :py:mod:`smrt.core.result.Result` instance with the results. Contact the core
